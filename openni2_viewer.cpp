    ...
    while (!cloud_viewer_->wasStopped () && (image_viewer_ && !image_viewer_->wasStopped ()))
    {
      ...
      if (cloud)
      {
        ...
        if (!cloud_viewer_->updatePointCloud (cloud, "OpenNICloud"))
        {
            ...
            pcl::visualization::PointCloudColorHandlerCustom<PointType> blue(cloud_plane, 0, 255, 0);
            ...
            std::cerr << "PointCloud representing the planar component: " << cloud_rest->width * cloud_rest->height << " data points." << std::endl;
            extract.setNegative (false);
            extract.filter (*cloud_abovePlane);
            filtered.swap (cloud_abovePlane);
            pcl::visualization::PointCloudColorHandlerCustom<PointType> yellow(cloud_plane, 0, 255, 0);
